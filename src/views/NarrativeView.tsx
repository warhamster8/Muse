...
export const NarrativeView: React.FC = () => {
  ...
  return (
    <div className="flex h-full gap-6 overflow-hidden animate-in fade-in duration-700">
      ...
      <div className="w-80 bg-white/[0.02] border border-white/5 rounded-[32px] overflow-hidden flex flex-col shadow-sm">
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
          <div className="flex items-center gap-2">
            <Library className="w-4 h-4 text-emerald-500/50" />
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">Manoscritto</span>
          </div>
          <button onClick={() => setModalType('chapter')} className="p-2 hover:bg-emerald-500/10 rounded-xl text-emerald-500 transition-colors border border-transparent hover:border-emerald-500/20">
            ...
            {chapters.map(chapter => (
              <div key={chapter.id} className="space-y-1">
                ...
                >
                  ...
                  <Folder className={cn("w-4 h-4 transition-colors", expandedChapters.has(chapter.id) ? "text-emerald-500" : "text-emerald-900")} />
                  <span className="text-sm font-medium text-slate-300 flex-1 truncate">{chapter.title}</span>
                  <Plus onClick={(e) => {
                    e.stopPropagation();
                    setTargetChapterId(chapter.id);
                    setModalType('scene');
                  }} className="w-3.5 h-3.5 text-slate-600 opacity-0 group-hover:opacity-100 hover:text-emerald-400 transition-all" />
                </div>
                          ...
                          <Draggable key={scene.id} draggableId={scene.id} index={index}>
                            {(provided, snapshot) => (
                              <div 
                                ...
                                className={cn(
                                  "flex items-center space-x-3 px-4 py-2 rounded-xl cursor-pointer transition-all group/scene border",
                                  ...
                                  snapshot.isDragging && "bg-slate-900 shadow-2xl border-emerald-500/30 z-50 scale-105"
                                )}
                              >
                                ...
                                <FileText className={cn("w-3.5 h-3.5", activeSceneId === scene.id ? "text-emerald-400" : "text-slate-600")} />
                                <span className={cn("text-xs truncate font-medium", activeSceneId === scene.id ? "text-slate-100" : "text-slate-500 group-hover/scene:text-slate-300")}>
                                  {scene.title}
                                </span>
                              </div>
                            )}
      ...
      <div className="flex-1 min-w-0 h-full relative">
        {activeScene ? (
          <div className="h-full bg-white/[0.01] rounded-[40px] border border-white/5 overflow-hidden shadow-inner">
            <Editor 
              key={activeScene.id} 
